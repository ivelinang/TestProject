	...
	double get_implied_ln_vol_skew_dampening(double expiry, double strike, double forward, double alpha, double beta, double rho, double nu)
	{
		...
		else if (std::abs(u) < epsilonATM_SABR_SkewDamping)
		{
			...
			double theta3 = 0.0;
			{
				...
				const double C = (a2 + a1 * b1 + b2) + (a3 + a2 * b1 + a1 * b2 + b3)*u 
					+ (a4 + a3 * b1 + a2 * b2 + a1 * b3 + b4)*u_squared 
					+ (b5 + a1 * b4 + a2 * b3 + a3 * b2 + a4 * b1 + a5)*u_cubed 
					+ (b6 + a1 * b5 + a2 * b4 + a3 * b3 + a4 * b2 + a5 * b1 + a6)*u_fourthed;
			...
			double xz02 = 0.0;
			{
				...
				xz02 = (z0 + rho_nu * 0.5*(z0_sq*u) + 0.5*(nu_squared)*(rho_sq - (1.0 / 3.0))*(z0*z0_sq*u_squared) 
					+ 0.125*(5.0*rho_sq - 3.0)*rho_nu*nu_squared*(z0_fr*u_cubed) 
					+ (1.0 / 40.0)*(nu_squared*nu_squared)*
			...
			double xzed = 0.0;
			{
				...
				const double z = A * (-1.0 + 0.5*beta*u - (1.0 / 6.0)*beta*(beta + 1.0)*u_squared 
					+ (1.0 / 24.0)*(beta*(beta + 1.0)*(beta + 2.0))*u_cubed 
					- (1.0 / 120.0)*(beta*(beta + 1.0)*(beta + 2.0)*(beta + 3.0))*u_fourthed);
				const double z_sq = z * z;
				const double z_fr = z_sq * z_sq;
				//const double z_fr = z_sq * z_sq;
				xzed = (z + rho_nu * 0.5*(z_sq + u) 
					+ 0.5*(nu_squared)*(rho_sq - (1.0 / 3.0))*(z*z_sq*u_squared) 
					+ 0.125*(5.0*rho_sq - 3.0)*rho_nu*nu_squared*(z_fr*u_cubed) 
					+ (1.0 / 40.0)*(nu_squared*nu_squared)*(3.0 - 30.0*rho_sq + 35.0*rho_sq*rho_sq)*(z*z_fr*u_fourthed));
...
} //namespace sabr model

namespace normal_sabr_model
{
	...
	double get_implied_normal_vol(double expiry, double moneyness, double alpha, double rho, double nu)
	{
		...
		else
		{
			...
			assert(expr >= 0, "negative value");

			const double big_expr = (sqrt(expr) - rho + z) / (1.0 - rho);
			assert(big_expr > 0.0, "negative value");

			const double xofz = log(big_expr);

			const double normal_vol = alpha * z / xofz * (1 + (2.0 - 3.0*rho*rho) / 24.0 *nu*nu*expiry);
	...
	{
		double normal_vol = get_implied_normal_vol(expiry, forward - strike, alpha, rho, nu);
...
namespace sabrd_model //displaces sabr
{
	double option_price(double expiry,
		double discount,
		double forward,
	...
	{
		if (expiry <= 0.0)
			return 1.0; //lognormal model with T=0
		else if (is_almost_zero(beta))
			return 1.0; //normal model

		else
		{
			return sabr_model::option_price(
				expiry,
				discount,
				forward - shift,
				strike - shift,
	...
	{
		assert(strikes.size() == results.size());

		auto res = results.begin();

		for (auto strike = strikes.begin(); strike != strikes.end(); ++strike, ++res)
		{
			*res = option_price(
				expiry,
				discount,
